...
---@field deterministicMapOrder boolean? Sorts encoded map keys for reproducible offline artifacts.

---@class BlizzardCBOREncodeState
---@field ignoreSerializationErrors boolean
---@field deterministicMapOrder boolean
...
function Encode.Table(tableValue, encodeState, tableDepth)
    ...
    if shape == "array" then
        ...
        if encodeState.deterministicMapOrder then
            local entries = {}
            for key, value in pairs(tableValue) do
                entries[#entries + 1] = {
                    encodedKey = Encode.MapKey(key, encodeState, tableDepth + 1),
                    value = value,
                }
            end
            ...
            for _, entry in ipairs(entries) do
                ...
                parts[#parts + 1] = Encode.Value(entry.value, encodeState, tableDepth + 1)
            end
        else
            for key, value in pairs(tableValue) do
...
function BlizzardCBOR.SerializeCBOR(...)
    ...
    local encodeState = {
        ...
        deterministicMapOrder = options ~= nil and options.deterministicMapOrder == true,
        activeTables = {},
    }
