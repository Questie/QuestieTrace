...
Load LibStub, `Libs/LibPopupStack-1.0/LibPopupStack-1.0.lua`, then `Widgets/Dialog.lua`. No widget XML or global mixin is required. WoW passes a private table to each addon file; the widget attaches its interface as `addon.Dialog`.
...
lua Tests/run.lua
busted -p '.test.lua' .
luacheck -q Widgets Modules/Consent.lua Tests/PopupUIHarness.lua Tests/run.lua
```

Tests construct the actual Lua hierarchy using native-control stand-ins. They cover independent addon namespaces sharing a stack, late layout changes, input, consent behavior, geometry, and coordinator upgrades. Native rendering, input, combat, and taint behavior still require client validation. Use disposable data for destructive consumer actions.
